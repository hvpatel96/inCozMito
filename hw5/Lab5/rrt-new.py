...
async def CozmoPlanning(robot: cozmo.robot.Robot):
    ...
    update, center = update, center = await detect_cube_and_update_cmap(robot, markers, cmap.get_start())
    if center is None:
    ...
    if center is None:
        while count < len(path) - 1:
            ...
            await robot.go_to_pose(cozmo.util.Pose(nextNode.x - currNode.x, nextNode.y - currNode.y, 0, angle_z=cozmo.util.Angle(degrees=np.arctan2(nextNode.y - currNode.y, nextNode.x - currNode.x))), relative_to_robot=True).wait_for_completed()
            count = count + 1
        if center is None:
            cmap.reset()
    ...
    RRT(cmap, cmap.get_start())
    path = cmap.get_smooth_path()
    await robot.turn_in_place(cozmo.util.degrees(-(angle % 360))).wait_for_completed()
    currentAngle = 0
    while count < len(path) - 1:
        currNode = path[count]
        nextNode = path[count + 1]
        ...
        # await robot.drive_straight(cozmo.util.distance_mm(final_distance), cozmo.util.speed_mmps(20)).wait_for_completed()

        # currentAngle = turn_angle
        # count = count + 1

        await detect_cube_and_update_cmap(robot, markers, cmap.get_start())
        if update:
            cmap.reset()
